...
class Package:
    ...
    def _fetch_sources(self, build_root, workspace, resource_dir, source_cache_dir):
        def checkout(self, source_url, cache_dir, workspace_dir):
            ...
            if not os.path.exists(workspace_dir):
                ...
                self.sh(
                    '%' + '{git} clone --local --shared 	"%s" "%s"' % (cache_dir, workspace_dir))
            ...
            if self.git_branch == None:
                ...
                self.sh('%' + '{git} fetch origin %s:refs/remotes/origin/%s' %
            ...
            if self.revision != None:
                ...
                target_revision = self.backtick(
                    '%' + '{git} rev-parse origin/%s' % self.git_branch)[0]
            ...
            if (self.revision != None and self.revision != current_revision):
                raise Exception('Workspace error: Revision is %s, package specifies %s' % (
                ...
                if source.startswith(('http://', 'https://', 'ftp://')):
                    ...
                    def checkout_archive(archive, cache, workspace):
                        ...
                        if not os.path.exists(cache):
                            # since this is a fresh cache, the workspace copy
                            # is invalid if it exists
                        ...
                        if not os.path.exists(workspace):
                            raise Exception(
                                'Archive %s was extracted but not found at workspace path %s' % (cache, workspace))
                        self.popd()
    ...
    def start_build(self, install_root, stage_root, arch):
        ...
        build_artifact = os.path.join(
            profile.build_root, self.name + '.artifact')
        ...
        if self.is_successful_build(build_artifact) and not (arch == 'darwin-universal' and self.needs_lipo):
            ...
            try:
                ...
                if arch == 'darwin-universal' and self.needs_lipo:
                    ...
                    self.do_build(
                        'darwin-32', install_root, workspace_x86, workspace_x86 + '.install')
                    stagedir_x86 = self.staged_prefix

                    self.do_build(
                        'darwin-64', install_root, workspace_x64, workspace_x64 + '.install')
                    stagedir_x64 = self.staged_prefix

                    stagedir_lipo = os.path.join(
                        profile.build_root, 'stage-lipo')
                    ...
                    try:
                        ...
                        self.lipo_dirs(
                            stagedir_x64, stagedir_x86, stagedir_lipo, 'lib')
                        run_shell('rsync -a --ignore-existing %s/* %s' %
                                  (stagedir_x86, profile.staged_prefix), False)
                    ...
                    self.do_build(
                        'darwin-32', install_root, workspace, stage_root)
                else:
    ...
    def do_build(self, arch, install_prefix, workspace_dir, stage_root):
        try:
            ...
            if os.path.exists(workspace_dir):
                ...
                shutil.rmtree(
                    os.path.join(self.profile.root,  problem_dir), ignore_errors=True)
                shutil.move(workspace_dir,
                            os.path.join(self.profile.root,  problem_dir))
            warn(str(e))
            error('Failed build at ./%s \n Run ./%s first to replicate environment for debugging.' %
                  (problem_dir, os.path.basename(self.profile.envfile)))
    ...
    def sh(self, *commands):
        for command in commands:
            ...
            full_command = '%s  > %s 2> %s' % (
                env_command, stdout.name, stderr.name)
            try:
            ...
            for file in filelist:
                if file.endswith('.a') or file.endswith('.dylib') or file.endswith('.so'):
                    ...
                    if os.path.exists(dir32_file):
                        ...
                        warn("lipo: 32-bit version of file %s not found" %
                             file)
        ...
        for root, dirs, filelist in os.walk(src_dir):
            ...
            for file in filelist:
                ...
                if filetype.startswith('Mach-O'):
                    # FIXME: perhaps add suffix before any .'s
                    dest_file = os.path.join(dest_dir, relpath, file + suffix)
                    dest_orig_file = os.path.join(dest_dir, relpath, file)
                    if orig_suffix != None and os.path.exists(dest_orig_file):
                        shutil.move(
                            dest_orig_file, dest_orig_file + orig_suffix)
                        os.symlink(
                            os.path.basename(dest_orig_file + orig_suffix), dest_orig_file)
                ...
                for idx, token in enumerate(tokens):
                    if token.find(self.staged_prefix) == -1:
                        tokens[idx] = token.replace(
                            self.profile.prefix, self.profile.staged_prefix)
                    else:
                        tokens[idx] = token.replace(
                            self.staged_prefix, self.profile.staged_prefix)
                ...
                if filetype.startswith('Mach-O'):
                    ...
                    try:
                        staged_path = os.path.join(
                            self.profile.staged_prefix, dir, os.path.relpath(path, dir))
                        run_shell('install_name_tool -id %s %s' %
                                  (staged_path, path))
                    except:
                        warn('Staging failed for %s' %
                    ...
                    for line in libs:
                        ...
                        if rpath.find(self.profile.prefix) != -1:
                            ...
                            try:
                                run_shell(
                                    'install_name_tool -change %s %s %s' % (rpath, remap, path))
                            except:
                                warn('Staging failed for %s' %
                                     os.path.relpath(path, dir))
                                abort_staging(path)
