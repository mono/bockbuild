...
class DarwinProfile (UnixProfile):
    ...
    def __init__(self, prefix=None, m64=False, min_version=6):
        ...
        sdk_paths = (sdkroot + 'MacOSX10.%s.sdk' %
                     v for v in range(min_version, 20))  # future-proof! :P
        ...
        # GTK2_RC_FILES must be a ":"-seperated list of files (NOT a single
        # folder)
    ...
    def configure_gtk(self):
        ...
        for path in paths:
            ...
            if os.path.basename(path) == 'pango.modules':
                fp = open(
                    os.path.join(os.path.dirname(bundle_path), 'pangorc'), 'w')
                fp.write('[Pango]\n')
    ...
    def configure_gdk_pixbuf(self):
        path = os.path.join(
            self.bundle_res_dir, 'etc', 'gtk-2.0', 'gdk-pixbuf.loaders.in')
